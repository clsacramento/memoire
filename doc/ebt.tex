...
\newglossaryentry{mpls-glo}
{
  ...
  text={MPLS},
  description={
	MPLS a proposé une distinction explicite entre la périphérie et le cœur du réseau.  Les routeurs dans la périphérie inspectent les entêtes des paquets entrants rajoute une étiquette utilisée pour les transmissions dans le cœur du réseau. Cette commutation basée sur les étiquettes sont utiliser pour délivrer les paquets et répondre aux pré-requis des opérateurs, comme tunnel \gls{vpn}. ou l'ingénierie du trafic. Les étiquettes MPLS ont du sens seulement dans le cœur du réseau et sont complètement indépendantes des protocoles spécifique aux hôtes (IPv4, IPv6 ou autre).  MPLS n'a pas formalisé l'interface entre interface de contrôle pour les opérateurs. Ainsi, MPLS a distingué les réseaux hôtes des interfaces de commutation des paquets, mais n'a pas développé une interface générale pour les opérateur réseau. \cite{fabric}}
...
\newglossaryentry{dpi-glo}
{
  name=DPI,
  text={DPI},
  description={ DPI est une technologie de filtrage de paquets qui les examine au moment où ils passent par le point d'inspection pour rechercher une non conformité de protocole, virus, spam, intrusion ou autre critère défini. L'outil décide alors si le paquet doit être transmit à la destination, re-routé ou bloqué. DPI fourni des données pour permettre une automatisation du réseau, une conformité aux règles définies et d'autres fonctions de sécurité. \cite{DPISearchSDN}}
...
\newglossaryentry{compute}
{
  name=Compute,
  text=compute,
  description={ Le terme compute  a été utilisé tout au long du document pour faire référence à la capacité de calcul et traitement de l'information des serveurs physiques. C'est l'élément qui est virtualisé au moyen des hyperviseurs permettant la mutualisation de ces ressources entre différentes applications.}
}
...
\newglossaryentry{openstack}
{
  name=OpenStack,
  text=OpenStack,
  description={ OpenStack est un projet pour le développement d'une plateforme Cloud Computing \gls{opnesource}. Le projet a été initié par NASA et Rackspace en 2010, et actuellement a été rejoint par diverses entreprises telles que HP, Dell, Cisco etc.

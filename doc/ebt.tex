...
{
  name=Open Daylight,
  description={ Association initiée par Linux Foundation pour l'union des géants du marché réseau dans le but de développer un contrôleur SDN \gls{opnesource}, pour l'innover, l'encourager et pour permettre son adoption accélérée. \cite{OpenDaylight} }
}

\newglossaryentry{openstack}
{
  name=OpenStack,
  description={ OpenStack est un projet pour le développement d'une plateforme Cloud Computing \gls{opnesource}. Le projet a été initié par NASA et Rackspace en 2010, et actuellement a été rejoint par diverses entreprise telles que HP, Dell, Cisco etc.

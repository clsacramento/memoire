...
\gls{nfv} pousse les technologies de virtualisation\index{Virtualisation} à consolider les applications réseau sur des serveurs, switches et baies de stockage\index{Stockage} physiques et standard de l'industrie dans une couche logique\index{Abstraction} qui met en disposition des ressources réseau\index{Virtualisation du Réseau}. Une ressource réseau virtualisée représente une \gls{vnf}. Le principe des \glspl{vnf} peut augmenter la flexibilité à partager ressources et réduire les coûts\index{Coûts} de configuration et gestion. Un fournisseur de services peut mettre en disposition un ensemble d'applications et éléments d'infrastructure dans une plateforme qui fournit aux tenants\index{Tenants} les ressources dont ils ont besoin pour déployer leurs propres applications réseau adaptées à leurs objectifs métiers.
...
Le \gls{controlplane} dans SDN fonctionne en lien avec le système de gestion\index{Gestion} cloud afin de configurer dynamiquement des éléments réseaux pour s'adapter aux décisions des systèmes d'orchestration pour le changement d'utilisations des ressources. SDN fournit l'infrastructure\index{Infrastructure} nécessaire pour réaliser cette capacité de NFV étendue.
...
La figure \ref{cloud-sdn-nfv} affiche comment SDN et NFV s'intègrent dans une infrastructure Cloud\index{Cloud Computing} pour atteindre les objectifs de virtualisation du réseau\index{Virtualisation du Réseau}. \\
...
Le cas d'utilisation NFVIaaS est présenté comme étant essentiel pour la livraison\index{Mode de livraison} de services cloud. Dans ce scénario, le fournisseur de services peut les offrir en utilisant une infrastructure\index{Infrastructure} NFV (NFVI) d'un autre fournisseur de services. L'importance de cette capacité est qu'elle permet d'étendre la portée d'un fournisseur à des localisations où il ne maintien pas d'atouts physiques.
...
NFVIaaS offre un moyen pour que l'opérateur Z loue son infrastructure NFV (compute, stockage, réseau) à X, lui permettant l'accès à une infrastructure que serait autrement d'un coût\index{Coûts} prohibitif à obtenir.
...
NFVIaaS représente également la simplification du déploiement par l'abstraction\index{Abstraction} des différences parmi divers opérateurs, autrement gérées avec de la tunnelisation, la \gls{qos}, renforcement de politiques de sécurité et procédures opérationnelles. Autres besoins importants incluent l'aspect multi-tenant\index{Tenants} pour l'isolation suffisante du trafic\index{Trafic}, renforcement de politiques spécifiques aux tenants et la \gls{scalability}\index{Scalabilité}.

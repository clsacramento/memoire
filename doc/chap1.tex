...
\chapter{Du data centre au Cloud Computing}
\label{chap-1}
...
Traditionnellement les data centres d'entreprises sont conçus pour durer pour toujours et atteindre les objectives visibles du business. Cela veut dire que les éléments sous-jacents sont dimensionnés et construits pour supporter le pic de charge projeté en termes de performance, disponibilité et sécurité. Quand la croissance volumétrique projetée ne correspond pas à la réalité, cette méthode de dimensionnement peut conduire à une situation de sous-dimensionnement ou sur-dimensionnement. En général, pour atteindre une meilleure disponibilité, les infrastructures sont amenées à une sous-utilisation des ressources. Ce qui apporte un effet négatif pour les investissements et les effort de réduction de coûts.

Une des plus grandes difficultés techniques d'offrir un service en ligne est la capacité de gérer la flexibilité des requis. La charge des applications varient continument dans les business sur internet, donc les choix sont de soit sous-dimensionner la provision et perdre des clients ou sur-dimensionner et gaspiller les ressources. En général, un plan détaillé de capacité est fait et suit d'un investissement important en matériel et logiciel, étant donné que leur capacité est fixe. L'image suivant illustre cette planification et les situations de problèmes de dimensionnement.
...
Les défauts de ce modèle ont amené les tendances de conception d'infrastructures avec des services partagés et virtualisation pour augmenter l'efficacité de l'utilisation des ressources. Virtualisation fait référence à l'abstraction de ressources logiques de leurs couches physiques pour améliorer l'agilité, la flexibilité et la réduction des coûts et ainsi mettre en valeur le business. La virtualisatoin permet de consolider un ensemble de composants d'infrastructures sous-utilisés en un nombre de dispositifs plus petits et meilleur utilisés, contribuant significativement à l'économie des coûts.

La virtualisation de serveurs, en spécial, est une méthode d'abstraire le système d'exploitation de la plateforme matérielle. Cela permet des multiples systèmes exploitation ou multiples instances du même système d'exploitation de coexister dans un ou plus processeurs. 
